...
public class DXConfig<T extends Config>{
    ...
    public DXConfig(Class<T> type, String jsonFilePath, MessageRedirector redirector) {
        ...
        this.redirector = redirector == null ? (mes, head) -> System.err.printf("[%s][DXConfig] %s", head, mes) : redirector;
        this.configFile = new File(jsonFilePath);
        this.type = type;
        if(!this.configFile.exists()) {
            this.log("Config file %s couldn't be found, generating empty config", this.configFile.getPath());
            try{
                this.forceCreate();
                this.valid = true;
            }catch (IOException e) {
    ...
    private void log(String message) {
        this.redirector.log(message, "INFO");
    }
    ...
    private void log(String format, Object... args) {
        this.redirector.log(String.format(format, args), "INFO");
    }
    ...
    private void forceCreate() throws IOException {
        ...
        try {
            ...
            this.redirector.log("Error while creating config file", "INFO");
            e.printStackTrace();
            childSuccess = false;
        }
        if(!childSuccess) {
